...
#define PANIC(str, ...) { logf(LOG_LEVEL_ERR, "PANIC @ %s:%d: " str, __FILE__, __LINE__ __VA_OPT__(,) __VA_ARGS__); while(1); }
...
#define ASSERT_PAGE_ALIGNED(name, val) { if(!ARCH_IS_PAGE_ALIGNED(val)) PANIC(name " assertion failed, got %d", val); }
